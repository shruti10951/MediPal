        ...
        for (QueryDocumentSnapshot document in appointmentSnapshot.docs) {
          ...
          if (difference.inHours <= 24 && data['status'] == 'pending') {
            ...
            TwilioFlutter twilioFlutter = TwilioFlutter(
                accountSid: cred[0], authToken: cred[1], twilioNumber: cred[2]);
            ...
            String doctorName = EncryptionDecryption.decryptAES(data['doctorName']);
            String location = EncryptionDecryption.decryptAES(data['location']);
            String description = EncryptionDecryption.decryptAES(data['description']);
            String date =
                DateFormat('d MMM yyyy').format(appointmentDateTime).toString();
            String time =
                DateFormat.Hm().format(appointmentDateTime).toString();
              ...
              if (snapshots.docs.isNotEmpty) {
                for (QueryDocumentSnapshot snapshot in snapshots.docs) {
                  DependentModel dependentModel =
                      DependentModel.fromDocumentSnapshot(snapshot);
                  userData = dependentModel.toMap();
                }
              }
              var guardians =
                  await FirebaseCred().getGuardianData(userData['userId']);
              String msgForGuardian =
                  'This is a reminder of your dependent $userData["name"]\'s upcoming appointment: \nDoctor: $doctorName \nLocation: $location \nDescription: $description \nDate: $date \nTime: $time \n\nPlease ensure that you arrive on time. \nBest regards, \nMediPal : Your Own Reminder App';

              for (Map<String, dynamic> guardian in guardians) {
                await twilioFlutter.sendSMS(
                    toNumber: "+91" +
                        EncryptionDecryption.decryptAES(guardian['phoneNo']),
                    messageBody: msgForGuardian);
              }
              await twilioFlutter.sendSMS(
                  toNumber: "+91" +
                      EncryptionDecryption.decryptAES(userData['phoneNo']),
                  messageBody: msg);
